...
;; Custom filter to render markdown like GitHub
...
;; Show preview of current markdown buffer
...
(defun my-org-html-filter-2 (buffer)
  (princ
   (with-temp-buffer
     (insert-buffer-substring buffer)
     (shell-command-on-region (point-min) (point-max) "pandoc -s -f org -t html5" nil t)
...
;; Show HTML preview of current org buffer
(defun my-org-html-preview ()
  ...
  (unless (process-status "httpd")
    (httpd-start))
  (impatient-mode)
  (imp-set-user-filter 'my-org-html-filter-2)
  (imp-visit-buffer))
#+END_SRC
